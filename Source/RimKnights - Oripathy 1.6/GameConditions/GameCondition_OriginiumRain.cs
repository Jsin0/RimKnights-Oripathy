...
namespace RimKnights.Oripathy
{
    public class GameCondition_OriginiumRain : GameCondition_ForceWeather
    {
        ...
        {
            base.Init();
            GameConditionExtension modExtension = this.def.GetModExtension<GameConditionExtension>();
            if (modExtension != null) 
            {
                ...
                compMutableActivityFactor = modExtension.compMutableActivityFactor;
            }
        }
        ...
        public override void GameConditionTick()
        {
            ...
            for (int i = 0; i < this.overlays.Count; i++)
            {
                ...
                {
                    this.overlays[i].TickOverlay(affectedMaps[j], 1f);
                }
            }
        }
        ...
        public static void DoPawnOriDamage(Pawn p, bool protectedByRoof = false, bool protectedIndoors = false, float damageMultiplier = 1f)
        {
            ...
            if (num != 0f)
            {
                ...
                num *= num2;
                HediffDef hediff;
                if (ModsConfig.IsActive("RimKnights.Oripathy")){
                    hediff = HediffDefOf.RK_OriginiumBuildup;
                }
                else
                {
                    hediff = RimWorld.HediffDefOf.ToxicBuildup;
                }
                HealthUtility.AdjustSeverity(p, hediff, num);
            }
        }
        ...
        public float compMutableActivityFactor = 1f;
    }
}
